...
grep(c("mean|std"),features$X2)
measure <- mergedata[,grep(c("mean|std"),features$X2)]

#step 3:Uses descriptive activity names to name the activities in the data set
activity <- read_delim('UCI HAR Dataset/activity_labels.txt',col_names = F)
newcode <- activity$X2
mergedata$activity <- newcode[mergedata$activity]

#step 4:Appropriately labels the data set with descriptive variable names
names(mergedata)[3:563] <- features$X2
